...
(* Why3 assumption *)
Definition eqPartition (p1:partition) (p2:partition) : Prop :=
  ((max p1) = (max p2)) /\
...
Axiom twopart :
  forall (p1:partition) (p2:partition) (x:Numbers.BinNums.Z)
    (y:Numbers.BinNums.Z),
  isPartition p1 -> isPartition p2 -> eqPartition p1 p2 ->
  (0%Z <= x)%Z /\ (x < (max p1))%Z -> (0%Z <= y)%Z /\ (y < (max p1))%Z ->
  eqPartition (punion p1 (pfind x p1) (pfind y p1))
...
Axiom cons'spec : True.

Axiom listLength'spec : True.

Parameter maxWeightConst: Numbers.BinNums.Z.

Axiom maxWeight_pos : (0%Z < maxWeightConst)%Z.

Axiom edge : Type.
Parameter edge_WhyType : WhyType edge.
Existing Instance edge_WhyType.

Parameter startVertex: edge -> Numbers.BinNums.Z.

Parameter endVertex: edge -> Numbers.BinNums.Z.

Parameter weight: edge -> Numbers.BinNums.Z.

Axiom edge'invariant :
  forall (self:edge),
  (0%Z <= (weight self))%Z /\ ((weight self) < maxWeightConst)%Z.

Axiom graph : Type.
Parameter graph_WhyType : WhyType graph.
Existing Instance graph_WhyType.

Parameter numVertices: graph -> Numbers.BinNums.Z.

Parameter edges: graph -> array edge.

Axiom graph'invariant :
  forall (self:graph),
  ((numVertices self) = (length (edges self))) /\
  (forall (i:Numbers.BinNums.Z),
   (0%Z <= i)%Z /\ (i < (numVertices self))%Z ->
   (let e := get (edges self) i in
    ((0%Z <= (startVertex e))%Z /\ ((startVertex e) < (numVertices self))%Z) /\
    (0%Z <= (endVertex e))%Z /\ ((endVertex e) < (numVertices self))%Z)).

Parameter hasVertex: graph -> Numbers.BinNums.Z -> Init.Datatypes.bool.

Axiom hasVertex'def :
  forall (g:graph) (v:Numbers.BinNums.Z),
  ((0%Z <= v)%Z ->
   ((v < (numVertices g))%Z -> ((hasVertex g v) = Init.Datatypes.true)) /\
   (~ (v < (numVertices g))%Z -> ((hasVertex g v) = Init.Datatypes.false))) /\
  (~ (0%Z <= v)%Z -> ((hasVertex g v) = Init.Datatypes.false)).

Axiom graph_edges_within :
  forall (g:graph),
  let es := edges g in
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i < (length es))%Z ->
  let e := get es i in
  ((hasVertex g (startVertex e)) = Init.Datatypes.true) /\
  ((hasVertex g (endVertex e)) = Init.Datatypes.true) /\
  (0%Z <= (weight e))%Z /\ ((weight e) < maxWeightConst)%Z.
...
(* Why3 assumption *)
Inductive heap :=
  | heap'mk : t1 (set.Fset.fset reference) -> t1 partition ->
      t1 Numbers.BinNums.Z -> t1 reference -> t1 (array Numbers.BinNums.Z) ->
...
Definition agree_allfields (sl:state) (sr:state) (pi:t2)
  ...
  agree_part sl sr pi w /\ agree_count sl sr pi w /\ agree_id sl sr pi w.
...
Definition ufPub (s:state) : Prop :=
  (forall (p:reference), isAllocated s p -> hasUfindType s p ->
   set.Fset.mem p (pool s) -> okUfind s p) /\
  (forall (p:reference), isAllocated s p -> hasUfindType s p ->
   set.Fset.mem p (pool s) ->
   set.Fset.mem (mixfix_lbrb (id (heap1 s)) p)
   (mixfix_lbrb (rep (heap1 s)) p)) /\
...
Axiom Requires : hasUfindType s self.

Axiom Requires1 : ~ (self = nullConst).

Axiom Requires2 : set.Fset.mem self (pool s).

Axiom H : (0%Z <= x)%Z.

Axiom H1 : (x < (max (mixfix_lbrb (part (heap1 s)) self)))%Z.

Axiom H2 : (0%Z <= y)%Z.

Axiom H3 : (y < (max (mixfix_lbrb (part (heap1 s)) self)))%Z.

Axiom Requires3 : okUfind s self.

Axiom Requires4 : ufPub s.

Axiom Requires5 : ufPriv s.

Axiom Assert : hasIntArrayType s nullConst.

Axiom Ensures :
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i < 0%Z)%Z ->
  ((get (make 0%Z 0%Z) i) = 0%Z).

Axiom Ensures1 : ((length (make 0%Z 0%Z)) = 0%Z).

Axiom Ensures2 :
  ((mixfix_lbrb (id (heap1 s)) self) = (find self (to_fmap1 (id (heap1 s))))).

Parameter ina: reference.

Axiom Ensures3 : (ina = (mixfix_lbrb (id (heap1 s)) self)).

Axiom Ensures4 :
  ((mixfix_lbrb (slots (heap1 s)) ina) =
   (find ina (to_fmap1 (slots (heap1 s))))).

Parameter sid: array Numbers.BinNums.Z.

Axiom Ensures5 : (sid = (mixfix_lbrb (slots (heap1 s)) ina)).

Axiom Ensures6 :
  set.Fset.subset (set.Fset.union (pool s) (img_rep s (pool s)))
  (set.Fset.union (pool s) (img_rep s (pool s))).

Axiom Ensures7 : True.

Axiom Ensures8 : okUfind s self.

Axiom Ensures9 : ufPub s.

Axiom Ensures10 : ufPriv s.

Parameter xroot: Numbers.BinNums.Z.

Axiom Ensures11 : mem1 xroot (pfind x (mixfix_lbrb (part (heap1 s)) self)).

Axiom H4 : (0%Z <= xroot)%Z.

Axiom H5 : (xroot < (max (mixfix_lbrb (part (heap1 s)) self)))%Z.

Axiom Ensures12 :
  ((get (mixfix_lbrb (slots (heap1 s)) (mixfix_lbrb (id (heap1 s)) self))
    xroot)
   = xroot).
...
Axiom Ensures14 : True.

Axiom Ensures15 : okUfind s self.

Axiom Ensures16 : ufPub s.

Axiom Ensures17 : ufPriv s.

Parameter yroot: Numbers.BinNums.Z.

Axiom Ensures18 : mem1 yroot (pfind y (mixfix_lbrb (part (heap1 s)) self)).

Axiom H6 : (0%Z <= yroot)%Z.

Axiom H7 : (yroot < (max (mixfix_lbrb (part (heap1 s)) self)))%Z.
...
Axiom Ensures21 : True.

Axiom Ensures22 : okUfind s self.

Axiom Ensures23 : ufPub s.

Axiom Ensures24 : ufPriv s.

Parameter conn: Init.Datatypes.bool.

Axiom Ensures25 :
  (conn = Init.Datatypes.true) <->
  mem1 y (pfind x (mixfix_lbrb (part (heap1 s)) self)).

Axiom H8 : ~ (conn = Init.Datatypes.true).

Axiom Ensures26 :
  ((mixfix_lbrb (part (heap1 s)) self) =
   (find self (to_fmap1 (part (heap1 s))))).

Parameter prt: partition.

Axiom Ensures27 : (prt = (mixfix_lbrb (part (heap1 s)) self)).
...
Axiom Ensures28 :
  ((to_fmap1 (part s_heap)) =
   (add self (punion prt (pfind x prt) (pfind y prt))
    (to_fmap1 (part (heap1 s))))).

Axiom H9 : ((rep s_heap) = (rep (heap1 s))).

Axiom H10 : ((count s_heap) = (count (heap1 s))).

Axiom H11 : ((id s_heap) = (id (heap1 s))).

Axiom H12 : ((slots s_heap) = (slots (heap1 s))).

Axiom H13 : ((length1 s_heap) = (length1 (heap1 s))).

Parameter i: Numbers.BinNums.Z.

Axiom Ensures29 : (i = 0%Z).

Axiom Ensures30 :
  ((mixfix_lbrb (length1 s_heap) ina) =
   (find ina (to_fmap1 (length1 s_heap)))).

Parameter len: Numbers.BinNums.Z.

Axiom Ensures31 : (len = (mixfix_lbrb (length1 s_heap) ina)).

Axiom Assert1 :
  forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < len)%Z ->
  ((get (mixfix_lbrb (slots s_heap) ina) j) = (get sid j)).

Parameter i1: Numbers.BinNums.Z.

Parameter s2: state.

Axiom H14 : ((rep (heap1 s2)) = (rep s_heap)).

Axiom H15 : ((part (heap1 s2)) = (part s_heap)).

Axiom H16 : ((count (heap1 s2)) = (count s_heap)).

Axiom H17 : ((id (heap1 s2)) = (id s_heap)).

Axiom H18 : ((length1 (heap1 s2)) = (length1 s_heap)).

Axiom H19 : ((pool s) = (pool s2)).

Axiom H20 : ((alloct s) = (alloct s2)).

Axiom H21 : hasIntArrayType s2 ina.

Axiom H22 : hasUfindType s2 self.

Axiom LoopInvariant : alloc_does_not_shrink s s2.

Axiom LoopInvariant1 : wrs_to_slots_framed_by s s2 (singleton ina).

Axiom LoopInvariant2 :
  forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < i1)%Z ->
  let inaj := get (mixfix_lbrb (slots (heap1 s2)) ina) j in
  ~ mem1 j (pfind x prt) ->
  mem1 j (pfind j prt) /\
  (inaj = (get sid j)) /\
  ((get (mixfix_lbrb (slots (heap1 s2)) ina) inaj) = inaj).

Axiom LoopInvariant3 :
  forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < i1)%Z ->
  ((get sid j) = xroot) ->
  ((get (mixfix_lbrb (slots (heap1 s2)) ina) j) = yroot).

Axiom LoopInvariant4 :
  forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < len)%Z ->
  ~ ((get sid j) = xroot) ->
  ((get (mixfix_lbrb (slots (heap1 s2)) ina) j) = (get sid j)).

Axiom LoopInvariant5 :
  forall (j:Numbers.BinNums.Z) (k2:Numbers.BinNums.Z),
  (0%Z <= j)%Z /\ (j < i1)%Z -> (0%Z <= k2)%Z /\ (k2 < i1)%Z ->
  mem1 j (pfind k2 prt) ->
  ((get (mixfix_lbrb (slots (heap1 s2)) ina) k2) =
   (get (mixfix_lbrb (slots (heap1 s2)) ina) j)).
...
Axiom LoopInvariant7 :
  forall (j:Numbers.BinNums.Z), (i1 <= j)%Z /\ (j < len)%Z ->
  ((get (mixfix_lbrb (slots (heap1 s2)) ina) j) = (get sid j)).

Axiom H25 : ~ (i1 < len)%Z.

Axiom Assert2 : (i1 = len).

Axiom Ensures32 :
  ((mixfix_lbrb (count (heap1 s2)) self) =
   (find self (to_fmap1 (count (heap1 s2))))).

Parameter cnt: Numbers.BinNums.Z.

Axiom Ensures33 : (cnt = (mixfix_lbrb (count (heap1 s2)) self)).

Parameter s3: state.

Axiom Ensures34 :
  ((to_fmap1 (count (heap1 s3))) =
   (add self (cnt - 1%Z)%Z (to_fmap1 (count (heap1 s2))))).

Axiom H26 : ((rep (heap1 s3)) = (rep (heap1 s2))).

Axiom H27 : ((part (heap1 s3)) = (part (heap1 s2))).

Axiom H28 : ((id (heap1 s3)) = (id (heap1 s2))).

Axiom H29 : ((slots (heap1 s3)) = (slots (heap1 s2))).

Axiom H30 : ((length1 (heap1 s3)) = (length1 (heap1 s2))).

Axiom H31 : ((pool s2) = (pool s3)).

Axiom H32 : ((alloct s2) = (alloct s3)).

Axiom H33 : hasUfindType s3 self.

Parameter x1: Numbers.BinNums.Z.

Parameter y1: Numbers.BinNums.Z.

Axiom H34 : (0%Z <= x1)%Z.

Axiom H35 :
  (x1 <
   (mixfix_lbrb (length1 (heap1 s3)) (mixfix_lbrb (id (heap1 s3)) self)))%Z.
...
Axiom H38 :
  ((get (mixfix_lbrb (slots (heap1 s3)) (mixfix_lbrb (id (heap1 s3)) self))
    x1)
   =
   (get (mixfix_lbrb (slots (heap1 s3)) (mixfix_lbrb (id (heap1 s3)) self))
    y1)).
...
(* Why3 goal *)
Theorem ufUnion'vc :
  ~ mem1 y1 (pfind x1 (mixfix_lbrb (part (heap1 s3)) self)).
Proof.
