...
let initial_tenv = {
  ctxt = IdentM.add (Id "alloc") T.Trgn IdentM.empty;
  ctbl = Ctbl.empty;
  grps = [];
  exts = []
}
...
type bi_tenv = {
  left_tenv: tenv;
  right_tenv: tenv;
  bipreds: bipred_params IdentM.t;
}
...
let initial_bi_tenv = {
  ...
  bipreds    = IdentM.empty;
}

let add_to_ctxt env id ty = { env with ctxt = IdentM.add id ty env.ctxt }
...
let show_tenv env =
  ...
  show_bindings (IdentM.bindings env.ctxt);
...
let is_array_ty = function
  | T.Tmath (Id "array", ty) -> Opt.is_some ty
  | _ -> false
...
let ity_of_ty_opt env = Opt.map (ity_of_ty env)
...
let qualify_tenv (env: tenv) (qual: string) : tenv =
  ...
  let ctxt =
    IdentM.fold (fun id ity new_ctxt ->
        IdentM.add
          (qualify_ident id (Some qual))
          (T.qualify_ity ity qual)
          new_ctxt
      ) ctxt IdentM.empty in
  let grps =
    List.map (fun (e,ls) ->
...
let tenv_union (env1: tenv) (env2: tenv) : tenv =
  let ctxt = IdentM.union (fun k v1 v2 -> Some v1) env1.ctxt env2.ctxt in
...
let bi_tenv_union (env1: bi_tenv) (env2: bi_tenv) : bi_tenv =
  let bipreds = IdentM.union (fun _ v _ -> Some v) env1.bipreds env2.bipreds in
...
type tpenv = tprogram_elt IdentM.t

let empty_tpenv : tpenv = IdentM.empty
...
let to_program_map (env: tpenv) : T.program_elt IdentM.t =
  IdentM.map (function
      | Typed (_, e, _) -> e
...
let find_in_ctxt env id loc =
  match IdentM.find id env.ctxt with
  | res -> ok res
  | exception Not_found ->
...
(* Why3 keywords that are not also source language keywords.  To keep
   ...
   NOTE! this restriction isn't placed on names for datagroups since
   these are expanded during compilation.  Therefore, image
   expressions can also contain these names.
*)
...
let add_quantifier_bindings env qbinds : (tenv * T.qbinders, string) result =
  List.fold_right (fun (id, ty, eopt) acc ->
      (* FIXME: location info is only approximate *)
      let* () = wf_ident ty.loc id in
      let* env, binders = acc in
      match ty.elt, eopt with
      | Tctor (cname, []), Some r when Ctbl.class_exists env.ctbl cname ->
        ...
        let binder = id -: ty', Some r' in
        ok (add_to_ctxt env id ty', binder :: binders)
      | _, None ->
        let* () = ensure_type_is_known env ty in
        let ty' = ity_of_ty env ty.elt in
        let binder = id -: ty', None in
        ok (add_to_ctxt env id ty', binder :: binders)
      | _, Some _ ->
        error_out (
...
let tc_meth_decl env mdecl : (tenv * T.meth_decl, string) result =
  ...
  let mdecl' =
    T.{meth_name = mdecl.elt.meth_name -: Tmeth {params=ptys'; ret=res_ty};
       params = params;
       result_ty = res_ty;
       result_ty_is_non_null = mdecl.elt.result_ty_is_non_null;
       meth_spec = spec'} in
  ok (env, mdecl')
...
let tc_extern env extern : (tenv * T.extern_decl, string) result =
  ...
  | Ex_type ->
    ...
    then error_out (
      ...
      let default = Opt.get extern.elt.extern_default in
  ...
  | Ex_function ->
    ...
    let ret = Opt.get extern.elt.extern_output in
  ...
  | Ex_const ->
    assert_is_some extern.elt.extern_output;
    let out = Opt.get extern.elt.extern_output in
...
let equal_meth_decl (m: meth_decl node) (m': meth_decl node)
  ...
  || m.elt.result_ty_is_non_null <> m'.elt.result_ty_is_non_null
  then error_out
      (Printf.sprintf
         "Return type mismatch: expected method %s to return %s%s"
         (string_of_ident m'.elt.meth_name)
         (string_of_ty m.elt.result_ty)
         (if m.elt.result_ty_is_non_null then "+" else ""))
      m'.elt.result_ty.loc
  else
...
let interface_does_import (intr_def: interface_def node) (name: ident) : bool =
  List.exists (fun e ->
      match e.elt with
      | Intr_import {elt=(Iregular, import_name, _); _} -> import_name = name
      | _ -> false
    ) intr_def.elt.intr_elts

let module_does_import (mdl_def: module_def node) (name: ident) : bool =
  List.exists (fun e ->
      match e.elt with
      | Mdl_import {elt=(Iregular, import_name, _); _} -> import_name = name
      | _ -> false
    ) mdl_def.elt.mdl_elts

let wf_import loc import : (unit,string) result =
  let (_, _, as_name) = import in
  match as_name with
  | None      -> ok ()
  | Some qual ->
    error_out (
      Printf.sprintf
        "Qualified import (... as %s) not allowed in this version\n"
...
let rec tc_interface penv tenv intr_def
  ...
  let walk penv env elt : (tpenv * tenv * T.interface_elt, string) result =
    ...
    | Intr_datagroup ds ->
      ...
      (* let ds' = List.map (fun d -> (d,[])) ds in
       * let new_env = {env with grps = ds' @ env.grps} in
       * ok (penv, new_env, T.Intr_datagroup ds) *)
    | Intr_formula nf ->
...
and tc_interface_import penv env intr_name import_decl
  ...
  let (kind, import, import_as) = import_decl.elt and loc = import_decl.loc in
  let* () = wf_ident loc import in
  let* () = wf_ident_opt loc import_as in
  let iname = string_of_ident import in
  let annot_import_decl = T.Intr_import (kind, import, import_as) in
  match kind with
  | Itheory ->
    ok (penv, env, T.Intr_import (Itheory, import, import_as))
  | Iregular ->
    match IdentM.find import penv with
    | Typed (_, T.Unary_interface idecl', Unary env') ->
      let combined_env = tenv_union_qualified env env' import_as in
      ok (penv, combined_env, T.Intr_import (kind, import, import_as))
    | Untyped (Unr_intr idecl' as intr_prog) ->
      assert (string_of_ident idecl'.elt.intr_name = iname);
      if interface_does_import idecl' intr_name
      then error_out (
          Printf.sprintf "Recursive imports between %s and %s \
                          not supported"
            iname (string_of_ident intr_name)
        ) loc
      else
        let* penv', env', idecl' =
        ...
        let combined_env = tenv_union_qualified env env' import_as in
        ok (penv', combined_env, annot_import_decl)
    | _
    | exception Not_found ->
...
let rec tc_module penv env mdl_def
  ...
  match IdentM.find mdl_name penv with
...
and tc_module_import penv env mdl_name import
  ...
  let {elt=(kind, import_name, import_as); loc} = import in
  let* () = wf_ident loc import_name in
  let* () = wf_ident_opt loc import_as in
  let iname = string_of_ident import_name in
  let annot_import = T.Mdl_import (kind, import_name, import_as) in
  match kind with
  | Itheory -> ok (penv, env, annot_import)
  | Iregular ->
    match IdentM.find import_name penv with
...
and tc_rformula_prim env loc pred args : (T.rformula, string) result =
  ...
  let { left_tenv = lenv; right_tenv = renv; _ } = env in
  match IdentM.find pred env.bipreds with
  | lparams, rparams ->
  ...
  | exception Not_found ->
    error_out (Printf.sprintf "Unknown symbol %s" (string_of_ident pred)) loc
...
let tc_binamed_formula bienv nf : (bi_tenv * T.named_rformula, string) result =
  ...
  | `Predicate ->
    ...
    let bprds = IdentM.add nf.elt.biformula_name (ltys,rtys) bienv.bipreds in
...
let add_named_rformula (env: bi_tenv) (nf: named_rformula node) (* FIXME: UNUSED *)
  ...
  | `Predicate ->
    ...
    let ltys = map (ity_of_ty lenv) @@ get_elts lparams in
    let rtys = map (ity_of_ty renv) @@ get_elts rparams in
    let bipreds = IdentM.add nf.elt.biformula_name (ltys, rtys) env.bipreds in
    ok { env with bipreds }
...
let wf_bimeth_def loc penv lmdl rmdl bimdef : (unit,string) result =
  ...
  let check c1 c2 side =
    if eqf_command c1 (rw_command c2) then ok ()
    else error_out (
  ...
  if Opt.is_none cc then ok () else begin
    let cc = Opt.get cc in
    let meth_name = bimdecl.bimeth_name in
...
let rec tc_bimodule (penv: tpenv) (env: bi_tenv) (bimdl_def: bimodule_def node)
  ...
  let walk penv env elt : (tpenv * bi_tenv * T.bimodule_elt, string) result =
    ...
    | Bimdl_extern ex ->
      let* left_tenv,  _   = tc_extern env.left_tenv ex in
      let* right_tenv, ex' = tc_extern env.right_tenv ex in
...
and tc_bimodule_import penv env bimdl_name import
  ...
  let* () = wf_ident_opt loc import_as in
  let iname = string_of_ident import_name in
  let annot_import = T.Bimdl_import (kind, import_name, import_as) in
  match kind with
  | Itheory -> ok (penv, env, annot_import)
  | Iregular ->
    match IdentM.find import_name penv with
...
and tc_bimodule_inienv penv env left_mdl right_mdl bimdl_name =
  let limport = no_loc (Iregular, left_mdl, None) in
  let rimport = no_loc (Iregular, right_mdl, None) in
...
type known_names = {
  known_fields: ident IdentM.t;
  known_classes: ident IdentM.t;
  known_globals: ident IdentM.t;
}
...
let empty_known_names = {
  known_fields = IdentM.empty;
  known_classes = IdentM.empty;
  known_globals = IdentM.empty;
}
...
let unique_field_names intr_name known cdecl : (known_names, string) result =
  let rec walk known = function
    ...
    | {elt = {field_name; _}; loc} :: flds ->
      if IdentM.mem field_name known.known_fields
      then error_out
          (Printf.sprintf "Field %s has already been declared"
             (string_of_ident field_name))
          loc
      else
        let fields = IdentM.add field_name intr_name known.known_fields in
...
let unique_field_names_mdl mdl_intr_name known cdef
  ...
  let rec walk known = function
    ...
    | {elt = {field_name; _}; loc} :: flds ->
      if IdentM.mem field_name known.known_fields
      then
        let intr_name = IdentM.find field_name known.known_fields in
        ...
        let fields = IdentM.add field_name mdl_intr_name known.known_fields in
        walk {known with known_fields = fields} flds in
...
let unique_class_names intr_name known cdecl : (known_names, string) result =
  let class_name = cdecl.elt.class_name in
  if IdentM.mem class_name known.known_classes
  then error_out
      (Printf.sprintf "Class %s has already been declared"
         (string_of_ident class_name))
      cdecl.loc
  else
    let known_classes = IdentM.add class_name intr_name known.known_classes in
...
let unique_class_names_mdl mdl_intr_name known cdef
  ...
  if IdentM.mem class_name known.known_classes
  then
    let intr_name = IdentM.find class_name known.known_classes in
    if intr_name = mdl_intr_name
    then ok known
    else error_out
        (Printf.sprintf "Class %s has already been declared"
    ...
    let classes = IdentM.add class_name mdl_intr_name known.known_classes in
...
let check_unique_names_interface known idef : (known_map, string) result =
  ...
  let rec walk known ielt =
    ...
    | Intr_vdecl (modif, id, ty) ->
      ...
      && IdentM.mem id known.known_globals
      then
        error_out (Printf.sprintf "Global %s has already been declared"
                     (string_of_ident id))
          ielt.loc
      else
        let globals = IdentM.add id intr_name known.known_globals in
...
let tc_program (prog: program)
  : (T.program_elt IdentM.t * Ctbl.t, string) result =
  ...
  let ini_penv =
    List.fold_left (fun map prog_elt ->
        ...
        IdentM.add (get_program_elt_name elt) (Untyped elt) map
      ) IdentM.empty prog in
  ...
  assert (IdentM.for_all (fun _ -> function Typed _ -> true | _ -> false) penv);
  if !tc_debug then begin
