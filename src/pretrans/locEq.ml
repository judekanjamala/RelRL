...
let local_equivalence ?(resolve=true) ctbl bnd spec =
  ...
  let agree_post =
    if resolve then agreement_effpost (all_fields ctbl) eff bnd
    else agreement_effpost [Ast.Id "any" -: Tdatagroup] eff bnd in
...
let rec derive_locEq ?(resolve=true) ctbl penv meth_name =
  ...
  local_equivalence ~resolve:resolve ctbl bnd spec
...
let pp_derive_locEq ?(resolve=true) ctbl penv meth_name outf : unit =
  let loc_eq = derive_locEq ~resolve:resolve ctbl penv meth_name in
